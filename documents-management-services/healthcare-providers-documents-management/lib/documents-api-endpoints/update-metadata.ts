...
import { updateMetadataLambda } from '../lambda-functions/documents/lambda-instances';

const resourcesNames = {
    lambda:  AWS_RESOURCES_NAMING_CONVENTION.replace('$', 'update-metadata'),
    iamRole: AWS_RESOURCES_NAMING_CONVENTION.replace('$', 'update-metadata-role'),
    iamPolicy: AWS_RESOURCES_NAMING_CONVENTION.replace('$', 'update-metadata-policy'),
    apiEndpoint: 'update-metadata',
    apiRequestModel: `${AWS_REQUETS_MODELS_NAMING_CONVENTION}UpdateMetadata`
};
export default function updateDocumentMetadataAction(
    scope: Construct, 
    ...
    const lambda = updateMetadataLambda(scope, lambdasFolder);
    const requestModel = {
        contentType: "application/json",
        description: `Validates parameters for Update Document Metadata API endpoint`,
